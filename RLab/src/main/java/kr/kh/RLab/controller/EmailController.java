...
public class EmailController {
    ...
    public byte[] sendEmail(HttpServletRequest request, @RequestParam("email") String email) throws UnsupportedEncodingException {
        ...
        emailService.sendEmail(email, "RLab 회원가입 인증 번호", "인증 번호: " + verificationCode);
	...
	public String findID(@RequestParam("email") String email) {
		...
		if (foundID != null) {
			emailService.sendEmail(email, "RLab 회원님의 아이디 찾기 결과", "회원님이 요청하신 아이디는 다음과 같습니다: " + foundID);
			return "found";
		} else {
			return "not found";
		}
	...
	public String findPW(@RequestParam("id") String id, @RequestParam("email") String email) {
		...
		if (memberVO != null) {
			...
			emailService.sendEmail(email, "RLab 회원님의 비밀번호 찾기 결과",
